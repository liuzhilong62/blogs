...
| 7 层探活 | 应用层数据库命令 | `testOnBorrow` / `testOnReturn` / `testWhileIdle` / `PING` | 视驱动而定，如 `SELECT 1`、`PING` |
| 7 层探活（主从） | 同上 | 需配置 test-query | `SELECT NOT pg_is_in_recovery()` / `SELECT @@READ_ONLY` |
...
| 组件 | 配置方式 |
|------|---------|
| PG server | `tcp_keepalives_idle`、`tcp_keepalives_interval`、`tcp_keepalives_count` |
| PG libpq | `keepalives`、`keepalives_idle`、`keepalives_interval`、`keepalives_count` |
| Redis server | 默认 OS 值，无独立配置 |
| redis-cli | 代码写死 `REDIS_CLI_KEEPALIVE_INTERVAL = 15` 秒 |
| Jedis | 依赖 OS 参数 |
| Lettuce | `clientOptions` 可配置 |
| Redisson | 无直接 keepalive，依赖 `pingConnectionInterval` |
...
- 4 层就够了：直连数据库，链路中无代理，TCP keepalive 配小即可
- 需要 7 层：链路中有代理、需要主从区分、需要确认数据库真的能执行 SQL（不只是 TCP 通）
...
连接池主动探活可以发现死连接，不同驱动/中间件的报错关键字：

| 技术栈 | 报错特征 |
|--------|---------|
| JDBC | `connection reset by peer`、`broken pipe`、`Connection is closed` |
| Netty（Lettuce/Redisson 底层） | `io.netty.channel` 开头异常 |
| libpq | `server closed the connection unexpectedly` |
| redis-cli | 无明显报错，TCP 层表现为持续 ACK 后发 RST |

**keepalive 不会中断正在运行的 SQL**——TCP keepalive 和 `tcp_retries2` 只在连接空闲时起作用，active 状态的连接有数据交互，keepalive 计时器不断被重置。
