...
FIN和RST都有正常情况和异常情况发的，其中要注意

1. 程序abort发的是FIN包，这包括KILL -9（我已测试验证ORACLE/PG进程kill -9 均发FIN包）
2. 端口不可达是RST包
3. tcp keepalive的中断也是RST包
4. 防火墙也有可能做RESET
5. RESET包跟业务层有`connection reset by peer`报错相关
...
**核心区分：FIN 是进程退出（内核替进程优雅关闭），RST 是网络不可达。**
