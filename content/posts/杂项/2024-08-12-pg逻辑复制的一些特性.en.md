...
**Summary: When replica identity is default, regardless of the operation (INSERT, UPDATE, DELETE), for old data, only the identity column is recorded; for new data, all non-TOAST columns are recorded, unless the TOAST column has a new value.**
...
> **⚠️ The above conclusion assumes no TOAST columns in the table.**
>
> When replica identity is FULL, WAL records the complete old tuple. If the old tuple contains TOAST columns (e.g., `text`, `bytea`, `jsonb` exceeding 2KB), PG calls `toast_flatten_tuple()` to **detoast and inline** the full TOAST data into WAL (source: `heapam.c:9129-9133`):
>
> ```c
...
>     {
>         *copy = true;
>         tp = toast_flatten_tuple(tp, desc);  // DETOAST!
>     }
>     return tp;
> }
> ```
>
> **With TOAST, FULL WAL volume can increase exponentially:**
>
> | Scenario | DEFAULT WAL | FULL WAL | Ratio |
> |----------|------------|----------|-------|
> | Table has no TOAST, UPDATE any column | ~1KB | ~2KB | ~2x |
> | Table has 10MB TOAST, UPDATE other col (TOAST unchanged) | ~1KB | **~10MB** | **~10,000x** |
> | Table has 10MB TOAST, UPDATE the TOAST column itself | ~10MB | ~20MB | ~2x |
> | DELETE row without TOAST | ~4B | ~100B | ~25x |
> | DELETE row with 10MB TOAST | ~4B | **~10MB** | **~2,500,000x** |
>
> Key insight: as long as the table has TOAST columns, FULL mode's old tuple will call `toast_flatten_tuple()` to detoast everything — regardless of whether the TOAST column was modified by the UPDATE. In DEFAULT mode, the old tuple only contains the PK, which never touches TOAST.
