...
> |----------|------------|----------|-------|
> | UPDATE non-TOAST columns | ~1KB | ~2KB | ~2x |
> | UPDATE other cols, 10MB TOAST unchanged | ~1KB | **~10MB** | **~10,000x** |
> | DELETE row without TOAST | ~4B | ~100B | ~25x |
> | DELETE row with 10MB TOAST | ~4B | **~10MB** | **~2,500,000x** |
>
> In DEFAULT mode, the old tuple only contains identity columns (PK), which are almost never TOAST types — so TOAST data is never written into old tuple WAL records.
