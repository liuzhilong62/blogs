...
> |------|------------|----------|------|
> | UPDATE 非 TOAST 列 | ~1KB | ~2KB | ~2x |
> | UPDATE 其他列, 10MB TOAST 不变 | ~1KB | **~10MB** | **~10,000x** |
> | DELETE 无 TOAST 列的行 | ~4B | ~100B | ~25x |
> | DELETE 含 10MB TOAST 的行 | ~4B | **~10MB** | **~2,500,000x** |
>
> 而 DEFAULT 模式下，old tuple 只包含标识列（PK），标识列几乎不可能是 TOAST 类型——因此 TOAST 数据永远不会被写入 old tuple 的 WAL 记录中。
