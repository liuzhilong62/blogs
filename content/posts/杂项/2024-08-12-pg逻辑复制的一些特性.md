...
date: 2024-08-12
categories: [杂项]
description: "深入分析PostgreSQL逻辑复制中replica identity对old/new值的影响，详解default与full模式下UPDATE/DELETE的数据差异"
---
...
**总结：当replica identity为default时，无论什么操作（INSERT,UPDATE,DELETE)，只要是old数据，只记录标识字段；只要是new数据，记录所有非toast字段，除非toast字段有new值**。

把default修改为full时，解析出来的日志量差别不会特别大，因为new数据永远是所有字段，（不考虑*全是*delete的场景下）full解析出的日志量小于default时的两倍。

> **⚠️ 以上结论的前提是：表中没有 TOAST 大字段。**
>
> 当 replica identity 为 FULL 时，WAL 会记录完整的 old tuple。如果 old tuple 包含 TOAST 列（如 `text`、`bytea`、`jsonb` 等超过 2KB 的大字段），PG 会调用 `toast_flatten_tuple()` 将 TOAST 数据**全量解压**写进 WAL（源码 `heapam.c:9129-9133`）：
>
> ```c
...
> ```
>
> **TOAST 场景下，FULL 的 WAL 日志量可能是指数级的：**
>
> | 场景 | DEFAULT WAL | FULL WAL | 倍数 |
> |------|------------|----------|------|
> | 表无 TOAST, UPDATE 任意列 | ~1KB | ~2KB | ~2x |
> | 表有 10MB TOAST, UPDATE 其他列(TOAST不变) | ~1KB | **~10MB** | **~10,000x** |
> | 表有 10MB TOAST, UPDATE TOAST 列本身 | ~10MB | ~20MB | ~2x |
> | DELETE 无 TOAST 列的行 | ~4B | ~100B | ~25x |
> | DELETE 含 10MB TOAST 的行 | ~4B | **~10MB** | **~2,500,000x** |
>
> 关键：只要表上有 TOAST 列，FULL 的 old tuple 就会调用 `toast_flatten_tuple()` 全量解压——无论该 TOAST 列是否被 UPDATE 修改。而 DEFAULT 模式下 old tuple 只有 PK，永远不会碰 TOAST。
